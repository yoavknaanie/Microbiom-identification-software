...
def copy_file_to_folder(file_path, folder_path):
    try:
        ...
        print(f"ERROR: the file path {file_path} is invalid")
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != INPUT_NUMBER:
        sys.exit("Invalid input. the input should be in the format: directory existing_fasta_path human_fasta_directory "
    ...
    # main function -----------------------------------------
    # shutil.copy(rf"{directory}\resources\fasta\Human_Top_100.fasta", chosen_fastas_folder) todo: delete
    ...
    if human_fasta_path != "none":
        human_fasta = rf"{chosen_fastas_folder}\human_proteome.fasta"
        combined_bacteria = rf"{chosen_fastas_folder}\combined_proteome.fasta"
        top20.combine_human_and_bacteria(human_fasta, combined_bacteria)
